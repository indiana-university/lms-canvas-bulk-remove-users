# lms-bulk-remove-users
Tool that helps bulk remove non-SIS enrollments in Canvas.
...
Insert a record into the `LTI_13_AUTHZ` table with your tool's registration_id (`lms_bulkremoveusers`), along with the client_id
...
Use an appropriate `canvas_user_login_id` and `canvas_course_id`.
...
<tr><td>Initiate login URL</td><td>http://localhost:8080/lti/login_initiation/lms_bulkremoveusers</td></tr>
